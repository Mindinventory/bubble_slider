import 'dart:math';
import 'package:demo/slider/widget.dart';
...
class BubbleSliderWidget extends LeafRenderObjectWidget {
  ...
  RenderObject createRenderObject(BuildContext context) {
    return _BalloonSliderRender(
        value: value,
        bubbleCurveLength: bubbleCurveLength,
        showRope: showBubble,
...
class _BalloonSliderRender extends RenderBox {
  _BalloonSliderRender(
      ...
      : assert(value >= 0.0 && value <= 1.0),
        ...
        _value = value,
        _color = color,
        _bubbleCurveLength = bubbleCurveLength,
        _showRope = showRope,
  ...
  double get bubbleCurveLength => _bubbleCurveLength;
  double _bubbleCurveLength;

  set bubbleCurveLength(double val) {
    assert(val >= 0);
    if (val == _bubbleCurveLength) {
      return;
    }
    _bubbleCurveLength = val;
    markNeedsPaint();
  }
  ...
  BubbleSliderState _state;
  bool _active = false;
  double _trackHeight = 8;
    ...
    if (showBubble) {
      for (int i = 0; i < _state.animationControllers.length; i++) {
        ...
        if (_state.animationControllers[i].value <= 178) {
          canvas.drawCircle(
            Offset(
                _state.bubblesList[i].cal, -(_state.bubblesList[i].yOffset / 2)),
            radius,
            Paint()..color = color,
          );
  ...
  Rect _getBalloonRect(
      ...
      Rect.fromLTWH(
          offset.dx,
          offset.dy - ((_bubbleHeight - parentSize.height) / 2) - bubbleCurveLength,
          _bubbleWidth,
          _bubbleHeight);
